...
internal final class ErrorReporterDeprecated: NSObject, URLSessionDelegate, URLSessionDataDelegate {
...
//        os_log("Posting error message of length %d", log: Self.log, type: .debug, messageData.count)
//
...
//        task.resume()
    ...
    func urlSessionDidFinishEvents(forBackgroundURLSession session: URLSession) {
        os_log("Finished posting error reporting events.", log: Self.log, type: .debug)
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        if let error = error {
            os_log("Error uploading reporting data: %@.", log: Self.log, type: .error, String(reflecting: error))
            return
        }

        if let httpResponse = task.response as? HTTPURLResponse, !(200...299).contains(httpResponse.statusCode) {
            os_log("Response status code != %d", log: Self.log, type: .error, httpResponse.statusCode)
            return
        }
    }
}
